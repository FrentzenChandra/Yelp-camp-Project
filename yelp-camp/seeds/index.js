...
const makeCampground = async () => {
  ...
  const random1000 = () => { return Math.floor(Math.random() * 1000) + 1};
  for (let i = 1; i <= 20; i++) {
    ...
    const description =
      "Lorem ipsum dolor sit, amet consectetur adipisicing elit. Minus repellendus natus laudantium tenetur fugiat cupiditate, et quod dolore perspiciatis nulla praesentium optio impedit ex doloribus rerum, libero consectetur distinctio qui?";
    const geometry = {
      type: "Point",
      coordinates: [cities[random1000()].longitude, cities[random1000()].latitude],
    };
    const campground = new Campground({ location, title, images, price, description, user, geometry });
